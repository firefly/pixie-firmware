...
static RlpStatus appendByte(RlpBuilder *rlp, uint8_t byte) {
    size_t remaining = rlp->length - rlp->offset;
    if (remaining < 1) { return RlpStatusBufferOverrun; }
    rlp->data[rlp->offset++] = byte;
    return RlpStatusOK;
}

static RlpStatus appendBytes(RlpBuilder *rlp, uint8_t *data, size_t length) {
    size_t remaining = rlp->length - rlp->offset;
    if (remaining < length) { return RlpStatusBufferOverrun; }
...
static RlpStatus appendHeader(RlpBuilder *rlp, uint8_t tag, size_t length) {
    ...
    RlpStatus status = appendByte(rlp, tag + 55 + byteCount);
    if (status) { return status; }

    for (int i = byteCount - 1; i >= 0; i--) {
        RlpStatus status = appendByte(rlp, length >> (8 * i));
        if (status) { return status; }
    }
...
void rlp_build(RlpBuilder *rlp, uint8_t *data, size_t length) {
    rlp->data = data;
    rlp->offset = 0;
    rlp->length = length;
}

RlpStatus rlp_appendData(RlpBuilder *rlp, uint8_t *data, size_t length) {
    if (length == 1 && data[0] <= 127) {
        return appendByte(rlp, data[0]);
    }

    RlpStatus status = appendHeader(rlp, TAG_DATA, length);
    if (status) { return status; }

    return appendBytes(rlp, data, length);
}

RlpStatus rlp_appendString(RlpBuilder *rlp, const char *data) {
    return rlp_appendData(rlp, (uint8_t*)data, strlen(data));
}

RlpStatus rlp_appendArray(RlpBuilder *rlp, size_t count) {
...
static size_t finalize(RlpBuilder *rlp) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (l == 0) { return RlpStatusOverflow; }
        rlp->offset += l;
        length += l;
    }
    ...
    RlpStatus status = appendHeader(rlp, TAG_ARRAY, length);
    if (status) { return 0; }
...
size_t rlp_finalize(RlpBuilder *rlp) {
